...
class _HomeShellState extends State<HomeShell> {
  ...
  final List<Widget> _screens = const [
    ...
    ExploreScreen(),
    BookmarksScreen(),
    TelemetryLabScreen(architectureName: "App 3: BLoC"),
  ];
    ...
    return Scaffold(
      ...
      bottomNavigationBar: NavigationBarTheme(
        data: NavigationBarThemeData(
          ...
          indicatorColor: const Color(0xFFFF5252).withAlpha(40),
          labelTextStyle: WidgetStateProperty.resolveWith((states) {
            if (states.contains(WidgetState.selected)) {
              return const TextStyle(color: Color(0xFFFF7A7A), fontSize: 11, fontWeight: FontWeight.bold);
            }
            return const TextStyle(color: Colors.white38, fontSize: 11);
          }),
          iconTheme: WidgetStateProperty.resolveWith((states) {
            if (states.contains(WidgetState.selected)) {
              return const IconThemeData(color: Color(0xFFFF5252));
            }
